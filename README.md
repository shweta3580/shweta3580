# Hi 👋, I'm Shweta Patil

### B.Tech Artificial Intelligence & Machine Learning Student | Aspiring Software Developer

🎓 Final-year B.Tech student specializing in **Artificial Intelligence & Machine Learning**  
💻 Interested in **AI/ML, Data Science, Backend & Full-Stack Development**  
🌱 Currently strengthening my skills in **Java, Python, SQL and Data Structures & Algorithms**  
🚀 Passionate about building practical, user-focused applications and continuously learning new technologies.

---

## 👩‍💻 About Me

- 🎓 Pursuing **B.Tech in Artificial Intelligence & Machine Learning**
- 📊 CGPA: **8.33 / 10**
- 💡 Interested in **Artificial Intelligence, Machine Learning and Software Development**
- 🔨 Experienced in developing projects using **Python, React, Flask, Node.js, Express and MySQL**
- 🧠 Currently improving my **Java and Data Structures & Algorithms** skills
- 📈 Interested in working on real-world problems using technology
- 🤝 Open to learning, collaboration and career opportunities

---

## 🛠️ Technical Skills

### 💻 Programming Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### 🌐 Web Development

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)

### 🤖 AI / ML & Data

- Machine Learning
- NumPy
- Pandas
- Matplotlib

### 🗄️ Database

![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### 🔧 Tools & Technologies

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)

---

## 🚀 Featured Projects

### ⭐ Store Rating Platform - Fullstack Application

A full-stack store rating platform that allows users to browse stores and submit ratings.

**Tech Stack:** React.js • Node.js • Express.js • MySQL • JWT • REST API

**Key Highlights:**
- User authentication using JWT
- Role-based authorization
- Store and rating management
- RESTful backend APIs
- MySQL database integration
- React-based user interface

🔗 [View Project](https://github.com/shweta3580)

---

### 🤖 CollegeAssist — Rule-Based College Information Chatbot

A full-stack chatbot designed to help students quickly access commonly required college information.

**Tech Stack:** React.js • Flask • Python • MySQL • REST API

**Key Highlights:**
- Interactive React chat interface
- Rule-based intent detection
- MySQL database integration
- Flask REST API
- Fallback handling for unsupported questions
- Fast responses to predefined college queries

🔗 [View Project](https://github.com/shweta3580/CollegeAssist)

---

### 🧠 ChillBot — Student Companion Chatbot

An AI-powered student companion designed to support students with learning, motivation, productivity and everyday assistance.

**Tech Stack:** Python • Streamlit • Flask • Gemini API • CSV

**Key Highlights:**
- Student-focused conversational assistant
- Study assistance and query handling
- Goal tracking and reminders
- Diary and history tracking
- Calendar and alarm functionality
- Text and voice interaction

🔗 [View Project](https://github.com/shweta3580/ChillBot-Student-Companion)

---

### 🌱 Carbon Emission Prediction

A machine learning project focused on predicting carbon emissions using relevant input features and data analysis techniques.

**Tech Stack:** Python • Machine Learning • Pandas • NumPy • Matplotlib

---

### 🔧 Pipe Design and Forecasting Model

A technical project developed to provide a practical solution using programming and application development concepts.

**Tech Stack:** Python • AI/ML

---

## 📜 Certifications

- 🏆 **NPTEL Certification — Machine Learning**
- ☁️ **ServiceNow Virtual Internship Program**
- 🇩🇪 **A1 Level German Language Certification**

---

## 💼 Internship & Learning Experience

- **Java Full Stack Development**
- **ServiceNow**


These experiences helped me gain exposure to software development, backend technologies, networking concepts and modern cloud/security practices.

---

## 📊 Areas of Interest

- 🤖 Artificial Intelligence & Machine Learning
- 📊 Data Science & Data Analytics
- 💻 Software Development
- 🌐 Full-Stack Development
- 🗄️ Database Management
- 🔐 Cloud & Security
- 🧩 Problem Solving & DSA

---

## 📚 Currently Learning

```text
Java
Data Structures & Algorithms
SQL
Backend Development
Machine Learning
Data Science
